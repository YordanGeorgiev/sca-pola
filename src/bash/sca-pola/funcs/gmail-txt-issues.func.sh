...
# v1.0.0
# ---------------------------------------------------------
# cat doc/txt/sca-pola/funcs/gmail-txt-issues.func.txt
# ---------------------------------------------------------
doGmailTxtIssues(){
   ...
   while read -r f ; do 
      ...
      for Email in $Emails; do (
         echo -e "running: \n"
         echo mutt -s "$sca_pola_project proj ::: $subj :::" -a "$f" -- "$Email" \< $f
         mutt -s "$sca_pola_project proj ::: $subj :::" -a "$f" -- "$Email" < $f
         echo -e "\n"
      );
      done
...
# eof file: src/bash/sca-pola/funcs/gmail-txt-issues.func.sh
