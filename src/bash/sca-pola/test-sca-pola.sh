#!/bin/bash 
# file: src/bash/sca-pola/test-sca-pola.sh
      ...
      doLogTestRunEntry 'START'
		while read -r test_file ; do (
            ...
            then
               ...
               $function_name
               doLog "INFO test-sca-pola loop exit_code: $exit_code" 
               doLogTestRunEntry 'INFO' ' '$(date "+%H:%M:%S")
		...
		done < <(find src/bash/sca-pola/tests -type f -name '*.sh')

	);
	done < <(cat $product_instance_dir/src/bash/sca-pola/tests/run-sca-pola-tests.lst)
...
doInit(){
   ...
   sleep_interval=${sleep_interval:=0}   # to slow down during debuging export sleep_iterval=3
}
#eof doInit
...
doLog(){
   ...
   test -t 1 && echo " [$type_of_msg] `date "+%Y.%m.%d-%H:%M:%S"` [sca-pola][@$host_name] [$$] $msg "
...
doSetVars(){
   ...
   . "$product_instance_dir/src/bash/sca-pola/funcs/log-test-run-entry.func.sh"
...
#----------------------------------------------------------
# Purpose:
# a simplistic tester for the sca-pola
#----------------------------------------------------------
#
#----------------------------------------------------------
...
#----------------------------------------------------------
#
# eof file: src/bash/sca-pola/test-sca-pola.sh
