#!/bin/bash 
# file: src/bash/sca-pola.sh doc at the eof file
...
doRunActions(){
   ...
   test -z "$actions" && doPrintUsage && doExit 0 
	...
	while read -d ' ' action ; do (
		...
		done < <(find src/bash/sca-pola/funcs -type f -name '*.sh')
...
doSetVars(){
   ...
   # set -x
	test -z "${sca_pola_project:-}" && doParseConfFile
	#test -z "${sca_pola_project:-}" || doSetUndefinedShellVarsFromCnfFile
   # sleep 20
